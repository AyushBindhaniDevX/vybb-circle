          ...
          <div className="flex-1 space-y-12 order-2 lg:order-1">
            ...
            >
              ...
              {user ? (
                <SeatSelector
                  ...
                  category={event.category} // CRITICAL FIX: Passing category here
                />
              ) : (
            ...
            <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="space-y-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between text-left">
                ...
                <Button variant="outline" className="rounded-full border-white/10 bg-white/5 hover:bg-violet-600 transition-all gap-2 font-black text-[10px] tracking-widest" onClick={() => window.open(`https://www.google.com/maps?q=${event.coordinates.lat},${event.coordinates.lng}`, '_blank')}>
